...
public class D3GeneratorDeepLabelsJSON extends D3GeneratorBase {
	...
	public String generateJSONforD3() throws OGVICModelsException {
		...
		for (int i = 0; i < goArray.length; i++) {
			...
			startNode = ModelUtils.tryReplaceWithCashedInstanceForSameURI_for_VISO_Resources(startNode, GraphicObjectX.class);
		...
		*/
		...
		try {
			...
			while (dlRelIt.hasNext()) {
				...
				//link.put("target", goMap.get(endNode));
				link.put("source_uri", startNode.getRepresentedResource().toString());
				link.put("target_uri", endNode.getRepresentedResource().toString());
				link.put("value", "1");
